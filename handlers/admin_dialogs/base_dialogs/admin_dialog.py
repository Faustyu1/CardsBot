...
async def message_to_mailing_handler(
    message: Message, message_input: Message, manager: DialogManager
):
    await manager.switch_to(AdminSG)


async def export_clicked(
...
async def get_statistics(dialog_manager: DialogManager, **kwargs):
    created_users_today = await get_users_count_created_by_date(
...
admin_dialog = Dialog(
    ...
    Window(
        ...
        Start(
            Const("Просмотр ссылок"), id="check_links", state=ViewRefLinkSG.link_list
        ),
