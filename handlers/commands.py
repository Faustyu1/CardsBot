...
from data.text import HELP_MESSAGE, PREMIUM_TEXT, PRIVACY_MESSAGE, WELCOME_MESSAGE, WELCOME_MESSAGE_PRIVATE
...
async def handler_bot_start(msg: Message, command: CommandObject):
    ...
    if user.check_bonus_available():
        ...
        if is_subscribe:
            ...
            await msg.answer("После подписки на каналы спонсоров нажмите чтобы получить бонус",
...
async def handler_bonus_command(msg: Message):
    ...
    if is_subscribe:
        ...
        temp_code = ''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=10))
        await msg.answer("После подписки на каналы спонсоров нажмите чтобы получить бонус",
...
async def premium_command(msg: Message):
    try:
        ...
        await msg.bot.send_message(msg.from_user.id, f"{PREMIUM_TEXT} Выберите способ оплаты премиума:",
        ...
        print(e)
        await msg.reply("Пожалуйста, напишите боту что-то в личные сообщения, чтобы я смог отправить информацию.")
