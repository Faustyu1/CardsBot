...
def make_card_keyboard(card_id: int, total_cards: int):
    ...
    if card_id > 1:
        nav_buttons.append(
            types.InlineKeyboardButton(text="⬅️", callback_data=f"market:prev:{card_id}")
        )
    if card_id < total_cards:
        nav_buttons.append(
            types.InlineKeyboardButton(text="➡️", callback_data=f"market:next:{card_id}")
        )
    if nav_buttons:
    ...
    builder.row(
        types.InlineKeyboardButton(
            text="Купить 💎", callback_data=f"market:buy:{card_id}"
        )
    )
...
async def show_market_menu(message: types.Message):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text="ОТКРЫТЬ", callback_data="market:open"))

    await message.answer(
        "🏪 Биржа лимитированных карточек", reply_markup=builder.as_markup()
    )
...
async def process_shop_callbacks(callback: types.CallbackQuery):
    ...
    available_cards = [
        card for card in await get_all_lcards() if card.buy_count < card.edition
    ]
    ...
    if action in ["prev", "next"]:
        ...
        if user_coins < card.price:
            await callback.answer(
                f"Недостаточно монет! Нужно: {card.price:,}", show_alert=True
            )
            return
        ...
        builder.row(
            types.InlineKeyboardButton(
                text="Подтвердить ✅", callback_data=f"market:confirm:{card.id}"
            ),
            types.InlineKeyboardButton(
                text="Отменить ❌", callback_data=f"market:cancel:{current_pos}"
            ),
        )
        ...
        await add_coins(
            callback.from_user.id, -card.price, callback.from_user.username, True
        )
