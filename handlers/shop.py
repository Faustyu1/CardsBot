...
async def shop(msg: Message):
    if msg.chat.type in ["group", "supergroup", "channel"]:
        ...
        return
    await msg.answer(
        text=shop_text, reply_markup=await shop_keyboard(), parse_mode="HTML"
    )
...
async def shop_callback(callback: CallbackQuery):
    ...
    if action == "boost":
        await callback.message.edit_text(
            text=booster_text, reply_markup=await boost_keyboard(), parse_mode="HTML"
        )
    elif action == "coins":
        await callback.message.edit_text(
            text=coins_text, reply_markup=await coins_keyboard(), parse_mode="HTML"
        )
    elif action == "back":
        await callback.message.edit_text(
            text=shop_text, reply_markup=await shop_keyboard(), parse_mode="HTML"
            ...
            if user and user.last_usage:
                ...
                if can_get_now:
                    ...
                    await callback.message.answer(
                        "Вы получили бустер на 1 час! Время ожидания сокращено."
                    )
            else:
                await callback.answer("Вы уже можете получить карточку!")
...
@shop_router.message(
    ...
    lambda message: message.successful_payment.invoice_payload.startswith("coins"),
)
async def handle_coins_payment(message: Message):
    ...
    try:
        ...
        await message.answer(
            "Ошибка обработки платежа: неверный формат payload. Обратитесь в поддержку"
        )
        return

    result = await add_coins(
        message.from_user.id, quantity, username=message.from_user.username, in_pm=True
    )
    ...
    if result:
        ...
        confirmation_text = confirmation_payment_text.replace(
            "{quantity}", str(quantity)
        )
        await message.answer(
            text=confirmation_text, parse_mode="HTML", reply_markup=builder.as_markup()
        )
    else:
...
async def diceplay(msg: Message):
    ...
    if user and user.last_dice_play:
        ...
        if time_since_last_throw < cooldown:
            remaining_time = (
                cooldown - time_since_last_throw
            ).total_seconds() // 60  # Учитываем всё время
            await msg.reply(
                text=dice_limit.format(int(remaining_time)), parse_mode="HTML"
            )
            return
    ...
    await msg.reply(text=dice_game.format(dice_value, dice_value), parse_mode="HTML")
